 ...
 **/
double profileProgress(int start, int target, int current) 
{
 return fabs((double) current - (double) start) / fabs((double) target - (double) start);
}
...
int profile(int profileType, int min, int max, int startTicks, int targetTicks, int currentTicks, float dvPeriod = 1.0/3.0) {
    ...
    switch(profileType) {
        default: case PROFILE_TRAPEZOID:
            multiplier = profileTrapezoid(startTicks, targetTicks, currentTicks, dvPeriod);
        case PROFILE_JERK:
            multiplier = profileJerk(startTicks, targetTicks, currentTicks, dvPeriod);

    }
    return (int)((max - min) * multiplier + min);
}
